...
parser.add_argument(
	'--doOTU',
	help = "If included, add percent OTU similarity to pick at. If not included, will do MED [Default: False]",
	required = False,
	default = False)
...
# Short cut for when things fail
...
if not SHORTCUTTWO:
	...
	#---
	var.write('PATHfiles=' + str(PATHWAY) + "\n")
	#---
	mapping_fps_string = ''
	n = 0
...
if not SHORTCUTTHREE:
	...
	for i in fasta_files:
		...
		open("doOTU.txt", 'w').write('doOTU='+str(doOTU))
		if TF_demulti == 'False': # Raw data
			...
			pwdRawData = PATHWAY.strip()+'/RawData.sh'
			print 'Executing RawData script' #TBD
			subprocess.call(['sh',str(pwdRawData)])
		else: # Demultiplexed already
			open("fasta_fps.txt", 'w').write('fasta='+str(i)) # write fp for fasta file; will be over-written each loop
			pwdDemulti = PATHWAY.strip()+'/DemultiplexedData.sh'
			print 'Executing demultiplexed script' #TBD
			os.system(str(pwdDemulti))
	#		subprocess.call(['sh',str(pwdDemulti)])
		pos += 1
...
pwdPartTwo = PATHWAY.strip()+'/Preprocessing_bash.sh'
subprocess.call(['sh',str(pwdPartTwo)])
